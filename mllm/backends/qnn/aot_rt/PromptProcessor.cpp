...
void PromptProcessor<T>::init_io() {
  ...
  for (int l = 0; l < config_.num_layers; ++l) {
    // K cache uses token-major layout: [B, H, L, D] (D contiguous per token)
    auto k_tensor = Tensor::empty({1, (int)config_.num_heads, config_.context_len - config_.ar_len, config_.head_dim},
  ...
  for (int l = 0; l < config_.num_layers; ++l) {
    // K Output
    // present_key is token-major: [B, H, S, D] where S=ar_len for prefill graphs
    auto k_tensor = Tensor::empty({1, (int)config_.num_heads, config_.ar_len, config_.head_dim}, config_.kv_dtype, kQNN);
